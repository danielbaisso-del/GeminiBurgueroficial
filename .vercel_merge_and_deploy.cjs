...
(async()=>{
  try{
    ...
    if(!found){ console.log('PR not found for branch', branch); process.exit(0); }
    ...
    const data = { name: 'gemini-burgueroficial', target: 'production', gitSource: { type: 'github', org: owner, repo: repo, ref: 'main' } };
